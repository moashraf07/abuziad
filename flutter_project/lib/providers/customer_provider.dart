...
class CustomerProvider extends ChangeNotifier {
  ...
  Future<void> settleAllPoints(int customerId) async {
    ...
    // Mark all log entries as settled (throws if RLS blocks)
    await _pointsDao.settleAllForCustomer(customerId);

    // Deduct the full unsettled amount from the customer balance
    try {
      await _dao.addPoints(customerId, -unsettled);
    } catch (e) {
      throw Exception(
        'تمت تسوية السجلات لكن فشل خصم النقاط ($unsettled) من رصيد العميل.\n'
        'السبب: $e',
      );
    }
