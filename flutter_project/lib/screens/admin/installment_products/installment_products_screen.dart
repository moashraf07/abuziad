...
class _ProductFormScreenState extends State<_ProductFormScreen> {
  ...
  Future<void> _importFromInventory(BuildContext context) async {
    // Normalize data to Map<String, dynamic> so we can handle both
    // warehouse Items and InstallmentProducts uniformly.
    List<Map<String, dynamic>> allItems;
    try {
      if (_storeType == AppConstants.storeInstallment || _storeType == 'main') {
        // Installment section → show warehouse items
        final itemDao = ItemDao();
        ...
        // Section-specific (electrical, clothing, mobiles, …) → show products
        // already in that section so admin can copy data easily.
        final products = await InstallmentProductDao()
            .getAll(storeType: _storeType, availableOnly: false);
