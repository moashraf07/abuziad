...
class _CustomerPointsScreenState extends State<CustomerPointsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      // Fetch points log
      final log = await context.read<CustomerProvider>().getPointsLog(_customer.id!);

      // Fetch fresh customer data for up-to-date points balance
      Customer? fresh;
      try {
        fresh = await context.read<CustomerProvider>().getById(_customer.id!);
      } catch (_) {}
  ...
  Future<void> _settle(CustomerPointsLog entry) async {
    ...
    try {
      await context
          ...
          .settlePointsEntry(entry.id!, _customer.id!, entry.pointsEarned);
      if (mounted) _load();
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _settleAll() async {
    ...
    if (confirmed != true || !mounted) return;
    try {
      await context.read<CustomerProvider>().settleAllPoints(_customer.id!);
      if (mounted) _load();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
...
class _PointsTable extends StatelessWidget {
  ...
  const _PointsTable({required this.log, required this.onSettle});
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          ...log.map((entry) => _PointsRow(entry: entry, onSettle: onSettle)),
          _TotalRow(log: log),
        ],
      ),
    );
...
class _PointsRow extends StatelessWidget {
  ...
  const _PointsRow({required this.entry, required this.onSettle});
