...
class _SuppliersScreenState extends State<SuppliersScreen> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) setState(() { _suppliers = all; _loading = false; });
  }
  ...
  List<Supplier> get _filtered {
    ...
    return _suppliers.where((s) =>
        ...
        (s.section?.toLowerCase().contains(q) ?? false)).toList();
  }
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => const SupplierComparisonScreen()),
            ),
          ),
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                _SummaryChip(
                  ...
                  label: 'مديونية: ${AppFormatters.formatCurrency(_suppliers.fold(0.0, (s, x) => s + x.debt))}',
                  color: Colors.red,
                ),
              ]),
            ),
                ...
                : _filtered.isEmpty
                    ? Center(
                        child: Column(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          const Text('لا يوجد موردون', style: TextStyle(color: Colors.grey, fontSize: 16)),
                        ]),
                      )
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, i) => _SupplierCard(
                          ...
                          dao: _dao,
                          onEdit: () => _showSupplierForm(existing: _filtered[i]),
  ...
  void _showSupplierForm({Supplier? existing}) {
    ...
    final balanceCtrl = TextEditingController(text: (existing?.balance ?? 0).toStringAsFixed(0));
    final debtCtrl = TextEditingController(text: (existing?.debt ?? 0).toStringAsFixed(0));
        ...
        child: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(existing == null ? 'إضافة مورد جديد' : 'تعديل بيانات المورد',
            ...
            TextFormField(
              controller: nameCtrl,
              decoration: const InputDecoration(labelText: 'اسم المورد *', prefixIcon: Icon(Icons.store)),
            ),
            ...
            Row(children: [
              Expanded(
                child: TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'الهاتف', prefixIcon: Icon(Icons.phone)),
                ),
              ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                  ...
                  try {
                    ...
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(existing == null ? 'تم إضافة المورد ✓' : 'تم تحديث بيانات المورد ✓'),
                ...
                child: Text(existing == null ? 'إضافة المورد' : 'حفظ التعديلات'),
              ),
            ),
          ]),
        ),
    ...
    return Card(
      ...
      child: InkWell(
        ...
        onTap: () => Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => _SupplierDetailScreen(supplier: supplier, dao: dao)),
        ).then((_) => onRefresh()),
        child: Padding(
          ...
          child: Row(children: [
            CircleAvatar(
              ...
              child: const Icon(Icons.store, color: Color(AppColors.primaryInt)),
            ),
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              ...
              Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                GestureDetector(
                  ...
                  child: const Icon(Icons.print_outlined, size: 18, color: Colors.green),
                ),
...
Future<void> _printPurchaseInvoice(BuildContext context, PurchaseInvoice inv) async {
  ...
  await PdfHelper.printReport(
    ...
    summaryRows: [
      ...
      if (inv.notes != null && inv.notes!.isNotEmpty) {'label': 'الملاحظات', 'value': inv.notes!},
    ],
  );
}
...
class _SupplierDetailScreenState extends State<_SupplierDetailScreen>
  ...
  Future<void> _loadInvoices() async {
    ...
    try {
      final inv = await InvoiceDao().getPurchaseInvoices(supplierId: widget.supplier.id!);
      if (mounted) setState(() { _invoices = inv; _loadingInvoices = false; });
    } catch (_) {
  ...
  Future<void> _loadReceipts() async {
    ...
    if (mounted) setState(() { _receipts = r; _loadingReceipts = false; });
  }
        ...
        Container(
          ...
          child: Row(children: [
            ...
            if (s.section != null) ...[
              ...
              const SizedBox(width: 4),
              Text(s.section!, style: const TextStyle(fontSize: 13, color: Colors.indigo)),
              const SizedBox(width: 16),
            ],
            if (s.debt > 0) ...[
            ...
            children: [
              _InvoicesTab(
                ...
                onAddInvoice: () => Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => PurchaseInvoiceScreen(supplier: s)),
                ).then((_) => _loadInvoices()),
              ),
              _PhotosTab(
                receipts: _receipts,
...
class _ProductsTab extends StatelessWidget {
  ...
  const _ProductsTab({
    required this.products, required this.loading,
    required this.supplier, required this.dao, required this.onChanged,
  });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading
          ? const Center(child: CircularProgressIndicator())
          : products.isEmpty
              ? const Center(child: Text('لا توجد منتجات لهذا المورد', style: TextStyle(color: Colors.grey)))
              : ListView.builder(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        subtitle: Text('آخر توريد: ${p.lastSuppliedAt}${p.unit != null ? " | الوحدة: ${p.unit}" : ""}'),
                        trailing: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.end, children: [
                          Text(AppFormatters.formatCurrency(p.unitPrice),
                              style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
                          if (p.notes != null)
                            Text(p.notes!, style: const TextStyle(fontSize: 11, color: Colors.grey)),
                        ]),
                        leading: const CircleAvatar(
                          backgroundColor: Color(0x1A0D1B4B),
                          child: Icon(Icons.inventory_2, color: Color(AppColors.primaryInt), size: 20),
                        ),
                        onLongPress: () async {
  ...
  void _showAddProduct(BuildContext context) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        padding: EdgeInsets.only(bottom: MediaQuery.of(ctx).viewInsets.bottom + 16, left: 20, right: 20, top: 20),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text('إضافة منتج للمورد', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          const SizedBox(height: 16),
          ...
          Row(children: [
            ...
            Expanded(child: TextFormField(
              controller: unitCtrl,
              decoration: const InputDecoration(labelText: 'الوحدة', hintText: 'قطعة / كيلو'),
            )),
          ]),
          const SizedBox(height: 10),
...
class _InvoicesTab extends StatelessWidget {
  ...
  const _InvoicesTab({
    ...
    required this.supplier, required this.onAddInvoice,
  });
          ...
          : invoices.isEmpty
              ? Center(
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const Text('لا توجد فواتير لهذا المورد', style: TextStyle(color: Colors.grey)),
                    const SizedBox(height: 16),
                    ElevatedButton.icon(
                      style: ElevatedButton.styleFrom(
                      ...
                      itemBuilder: (ctx, i) {
                        ...
                        return Card(
                          child: ListTile(
                            ...
                            subtitle: Text(inv.date, style: const TextStyle(fontSize: 12)),
                            trailing: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                              Text(AppFormatters.formatCurrency(inv.total),
                                  style: const TextStyle(
                                      fontWeight: FontWeight.bold, fontSize: 14, color: Colors.orange)),
                              if (inv.notes != null)
                                Text(inv.notes!, style: const TextStyle(fontSize: 10, color: Colors.grey)),
      ...
      builder: (_, ctrl) => Column(
        children: [
          ...
          const Padding(
            ...
            child: Row(children: [
              Expanded(flex: 3, child: Text('اسم المنتج', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.grey))),
              SizedBox(width: 8),
              SizedBox(width: 48, child: Text('الكمية', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.grey))),
              SizedBox(width: 8),
              SizedBox(width: 64, child: Text('السعر', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.grey))),
              SizedBox(width: 8),
              SizedBox(width: 72, child: Text('الإجمالي', textAlign: TextAlign.end, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.grey))),
            ]),
          ),
...
class _PhotosTab extends StatelessWidget {
  ...
  final VoidCallback onChanged;
  const _PhotosTab({
    required this.receipts, required this.loading,
    required this.supplier, required this.dao, required this.onChanged,
  });
    ...
    return Scaffold(
      ...
      body: loading
          ...
          : receipts.isEmpty
              ? Center(
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    Icon(Icons.photo_camera, size: 64, color: Colors.grey.shade300),
                    const SizedBox(height: 12),
                    const Text('لا توجد صور فواتير', style: TextStyle(color: Colors.grey)),
                    const SizedBox(height: 16),
                    ElevatedButton.icon(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Card(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        if (hasImage)
                          GestureDetector(
                            onTap: () => _viewImage(ctx, r.imagePath!),
                            child: ClipRRect(
                          ...
                          child: Row(children: [
                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Row(children: [
                                ...
                                const SizedBox(width: 4),
                                Text(r.date, style: const TextStyle(fontSize: 13, color: Colors.grey)),
                              ]),
                              if (r.notes != null) ...[
                                const SizedBox(height: 4),
    ...
    await showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => Padding(
        padding: EdgeInsets.only(
            bottom: MediaQuery.of(ctx).viewInsets.bottom + 16, left: 20, right: 20, top: 20),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
