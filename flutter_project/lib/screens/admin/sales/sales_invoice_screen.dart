...
class _SalesInvoiceScreenState extends State<SalesInvoiceScreen> {
  ...
  Future<void> _lookupAndAdd(String code) async {
    ...
    if (item != null && mounted) {
      context.read<SalesProvider>().addItem(
            item,
            priceType: _selectedCustomers.isNotEmpty
                ? (_selectedCustomers.first.priceType ?? 'retail')
                : 'retail',
          );
      _barcodeCtrl.clear();
  ...
  Future<void> _selectCustomer() async {
    ...
    if (selected != null &&
      ...
      setState(() => _selectedCustomers.add(selected));
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (_, controller) => _ProductPickerSheet(
          ...
          priceType: _selectedCustomers.isNotEmpty
              ...
              : 'retail',
          onAdd: (item) {
            context.read<SalesProvider>().addItem(item,
                priceType: _selectedCustomers.isNotEmpty
                    ? (_selectedCustomers.first.priceType ?? 'retail')
                    : 'retail');
          },
          ...
          child: sales.cart.isEmpty
              ? Center(
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const SizedBox(height: 8),
                    Text('اضغط على "اختر منتج" لإضافة أصناف',
  ...
  Widget _buildInlineProductPicker(SalesProvider sales) {
    final priceType = _selectedCustomers.isNotEmpty
        ? (_selectedCustomers.first.priceType ?? 'retail')
        : 'retail';
          ...
          Row(children: [
            Expanded(
              child: GestureDetector(
                onTap: () =>
                    setState(() => _productSource = 'inventory'),
                child: Container(
              ...
              if (_productSource == 'inventory') {
                ...
                final filtered = _productQuery.isEmpty
                    ...
                    : allItems
                        .where((i) =>
                            ...
                            (i.barcode?.contains(_productQuery) ??
                                false))
                        .toList();
                if (filtered.isEmpty) {
                  return Center(
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    final price = item.priceForType(priceType);
                    final inCart =
                        sales.cart.any((c) => c.item.id == item.id);
                    return _ProductPickerTile(
                      name: item.name,
                      subtitle:
                          'كمية: ${item.quantity.toStringAsFixed(0)}',
                      price: price,
                      inCart: inCart,
                ...
                final filtered = _productQuery.isEmpty
                    ...
                    : _storeProducts
                        .where((p) =>
                            ...
                            (p.category?.contains(_productQuery) ??
                                false))
                        .toList();
                if (filtered.isEmpty) {
                  return Center(
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    final inCart = sales.cart
                        .any((c) => c.item.id == itemConverted.id);
                    return _ProductPickerTile(
                      name: p.name,
                      subtitle: p.category ?? '',
            ...
            child: Builder(builder: (_) {
              if (_source == 'inventory') {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (ctx, i) {
                    final item = filtered[i];
                    final price = item.priceForType(widget.priceType);
                    final inCart =
                        sales.cart.any((c) => c.item.id == item.id);
                    return _ProductPickerTile(
                      name: item.name,
