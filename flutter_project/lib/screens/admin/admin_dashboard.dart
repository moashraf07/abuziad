...
class _AdminDashboardState extends State<AdminDashboard> {
  ...
  int _pendingReqsFor(String storeType) =>
      _pendingRequestsByDept[storeType] ?? 0;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      Map<String, int> pendingByDept = {};
      try {
        final reqs = await RequestDao().getAllWithCustomer(status: 'pending');
        for (final req in reqs) {
          final dept = (req['customer_store_type'] as String?) ??
              (req['store_type'] as String?) ?? '';
          if (dept.isNotEmpty) {
            pendingByDept[dept] = (pendingByDept[dept] ?? 0) + 1;
          }
        }
        pendingByDept['all'] = reqs.length;
      } catch (_) {}
                  ...
                  children: [
                    ...
                    if (auth.isAdmin || auth.canAccessDept(AppConstants.deptInstallment)) ...[
                      _SystemHeader(
                        ...
                        subtitle: 'منتجات التقسيط • طلبات العملاء • الأقساط',
                        icon: Icons.payment,
                        ...
                        child: GridView.count(
                          ...
                          children: [
                            ...
                            StatCard(
                              title: 'طلبات العملاء',
                              value: () {
                                final deptType = (!auth.isAdmin && auth.departmentType != null && auth.departmentType != AppConstants.deptAll)
                                    ? auth.departmentType!
                                    : AppConstants.deptInstallment;
                                final cnt = _pendingReqsFor(deptType);
                                return cnt > 0 ? '$cnt طلب' : 'عرض';
                              }(),
                              icon: Icons.inbox,
                              color: () {
                                final deptType = (!auth.isAdmin && auth.departmentType != null && auth.departmentType != AppConstants.deptAll)
                                    ? auth.departmentType!
                                    : AppConstants.deptInstallment;
                                return _pendingReqsFor(deptType) > 0 ? Colors.red : Colors.teal;
                              }(),
                              onTap: () {
                                if (!auth.isAdmin &&
                                    auth.departmentType != null &&
                    ...
                    for (final dept in dynamicDepts)
                      _DynamicDeptSection(
                        dept: dept,
                        pendingRequests: _pendingReqsFor(dept.storeType),
                      ),
...
class _DynamicDeptSection extends StatelessWidget {
  ...
  const _DynamicDeptSection({required this.dept, this.pendingRequests = 0});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _SystemHeader(
          title: dept.name,
          subtitle: 'المنتجات • الطلبات • الأقساط',
          icon: _deptIcon(),
          color: color,
        ),
        Padding(
          ...
          child: GridView.count(
            ...
            children: [
              ...
              StatCard(
                title: 'طلبات العملاء',
                value: pendingRequests > 0 ? '$pendingRequests طلب' : 'عرض',
                icon: Icons.inbox,
                color: pendingRequests > 0 ? Colors.red : Colors.teal,
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
