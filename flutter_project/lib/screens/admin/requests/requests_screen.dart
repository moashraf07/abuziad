...
class _RequestsScreenState extends State<RequestsScreen>
  ...
  String get _title {
    final label = AppConstants.storeLabels[widget.storeType];
    return label != null ? 'طلبات — $label' : 'طلبات العملاء';
  }
...
class _RequestsListState extends State<_RequestsList> {
  ...
  Future<void> _accept(Map<String, dynamic> r) async {
    ...
    _notifyCustomer(r, approved: true);
    _load();
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('✓ تم قبول الطلب وإشعار العميل'),
  ...
  Future<void> _reject(Map<String, dynamic> r) async {
    ...
    _notifyCustomer(r, approved: false, reason: reason);
    _load();
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('تم رفض الطلب وإشعار العميل'),
  ...
  void _notifyCustomer(Map<String, dynamic> r,
    ...
    PushNotificationService.sendToCustomer(
      ...
      body: approved
          ...
          : (reason != null && reason.isNotEmpty
              ? reason
              : NotifMsg.requestRejectedBody),
      type: 'request',
      referenceId: r['id'] as int,
