...
class _UsersTabState extends State<_UsersTab> {
  ...
  IconData _roleIcon(String role) {
    switch (role) {
      ...
      case 'partner': return Icons.people;
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('تم تحديث كود دخول ${user.name} بنجاح: $code'),
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: const [
                  DropdownMenuItem(value: AppConstants.roleAdmin, child: Text('مدير النظام')),
                  DropdownMenuItem(value: AppConstants.roleManager, child: Text('مدير')),
                  DropdownMenuItem(value: AppConstants.rolePartner, child: Text('شريك')),
                ],
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (role == AppConstants.roleAdmin && passwordCtrl.text.isEmpty) return;
                ...
                final loginCode = role != AppConstants.roleAdmin
                    ? '${DateTime.now().millisecondsSinceEpoch % 100000}'.padLeft(5, '0')
                    : null;
                ...
                if (context.mounted) {
                  ...
                  if (loginCode != null) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('تم إضافة المستخدم ✓ | كود الدخول: $loginCode'),
                        backgroundColor: Colors.green,
...
class _SystemTab extends StatelessWidget {
      ...
      children: [
            ...
            if (auth.isAdmin) ...[
              ...
              ListTile(
                ...
                onTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (_) =>
                          const SupabaseDiagnosticScreen()),
                ),
              ),
            ],
          ]),
        ...
        Card(
          ...
          child: const ListTile(
            ...
            subtitle: Text('نظام ERP متكامل للمتاجر الكهربائية ومتاجر التقسيط\nالإصدار 3.0.0'),
          ),
        ),
      ],
    );
  ...
  Future<void> _performBackup(BuildContext context) async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('بياناتك محفوظة على Supabase (السحابة) ويتم نسخها احتياطياً تلقائياً.'),
  ...
  void _showAboutDialog(BuildContext context) {
    showAboutDialog(
      ...
      children: const [
        ...
        SizedBox(height: 8),
        Text('يشمل: المبيعات، المشتريات، الأقساط، الخزنة، التقارير، العملاء، الشركاء، الموردين'),
      ],
    );
  }
  ...
  Future<String?> _createTemporaryAccessCode(BuildContext context, String code) async {
    try {
                ...
                children: [
                        ...
                        child: ElevatedButton.icon(
                          onPressed: creating
                              ...
                              : () async {
                                  ...
                                  final result = await _createTemporaryAccessCode(dialogContext, code);
                                  ...
                                  if (result != null) {
                                    ...
                                    if (dialogContext.mounted) {
                                      ScaffoldMessenger.of(dialogContext).showSnackBar(
                                        SnackBar(
                                          content: Text('تم إنشاء الكود بنجاح: $code'),
                                          backgroundColor: Colors.green,
                                        ),
                                      );
                                    }
                  ...
                  if (loading)
                    ...
                    const Align(
                      ...
                      child: Text('الأكواد الأخيرة', style: TextStyle(fontWeight: FontWeight.bold)),
                    ),
                    const SizedBox(height: 8),
                    ...codes.map((item) {
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (newCtrl.text != confirmCtrl.text) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text('كلمتا المرور غير متطابقتين'), backgroundColor: Colors.red));
                  return;
                }
                if (newCtrl.text.length < 4) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text('كلمة المرور قصيرة جداً'), backgroundColor: Colors.red));
                  return;
                }
                final auth = ctx.read<AuthProvider>();
