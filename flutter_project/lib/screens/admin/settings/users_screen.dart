...
class _StaffTabState extends State<_StaffTab> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) setState(() { _users = users; _departments = depts; _loading = false; });
  }
  ...
  void _handleAction(String action, User u) async {
    switch (action) {
      case 'delete':
        ...
        if (ok && u.id != null) { await _dao.delete(u.id!); _load(); }
        break;
      case 'toggle':
        if (u.id != null) {
  ...
  void _showCodeResult(
      User u, String code, String codeType, String? expiry) {
    showDialog(
      context: context,
      builder: (dlgCtx) => AlertDialog(
        title: Text('كود دخول: ${u.name}'),
          ...
          if (codeType == AppConstants.codeTypeTemporary && expiry != null)
            Container(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 6),
                Text(
                    'ينتهي في ${AppConstants.temporaryCodeHours} ساعة',
                    style: const TextStyle(
      ...
      if (!status.isGranted) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('يرجى السماح بالوصول لجهات الاتصال من إعدادات الهاتف')),
          );
        }
        return null;
      }
              ...
              onPressed: () async {
                try {
                  await _dao.update(u.copyWith(
                    ...
                    departmentType:
                        u.role == AppConstants.roleManager
                            ? selectedDeptType
                            : u.departmentType,
                  ));
                  Navigator.pop(ctx);
                ...
                onPressed: () async {
                  ...
                  if (contact != null) {
                    setS(() {
                      if (contact['name'] != null) {
                        nameCtrl.text = contact['name']!;
                        usernameCtrl.text = contact['name']!
                            .replaceAll(' ', '_')
                            .toLowerCase();
                      }
                      if (contact['phone'] != null) {
              ...
              DropdownButtonFormField<String>(
                ...
                items: const [
                  DropdownMenuItem(
                      ...
                      child: Text('مدير')),
                  DropdownMenuItem(
                      value: AppConstants.rolePartner,
                      child: Text('شريك')),
                ],
                      ...
                      try {
                        ...
                        if (codeType ==
                          ...
                          expiry = DateTime.now()
                              .add(Duration(
                                  hours:
                                      AppConstants.temporaryCodeHours))
                              .toIso8601String();
                        }
                        final user = User(
                          username: usernameCtrl.text.trim(),
                          passwordHash:
                              HashHelper.hashPassword(rawPassword),
                          role: role,
                          name: nameCtrl.text.trim(),
                        ...
                        if (id == -2) {
                          ...
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content: Text(_dao.lastError ?? 'اسم المستخدم موجود بالفعل'),
                                  backgroundColor: Colors.red),
                            );
                          }
                          return;
                        ...
                        if (id <= 0) {
                          ...
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content: Text('فشل إضافة المستخدم: ${_dao.lastError ?? 'تحقق من البيانات أو صلاحيات Supabase.'}'),
                                  backgroundColor: Colors.red),
                            );
                          }
                          return;
                        ...
                        PushNotificationService.broadcast(
                          title: 'مستخدم جديد',
                          body: "تم إضافة مستخدم جديد: ${nameCtrl.text.trim()} (${role == AppConstants.roleManager ? 'مدير' : 'شريك'})",
                        );
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
  ...
  IconData _roleIcon(String role) {
    switch (role) {
      ...
      case 'partner': return Icons.group;
...
class _CustomersTabState extends State<_CustomersTab> {
    ...
    showDialog(
      ...
      builder: (dialogCtx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min,
          ...
          if (c.phone != null)
            _DetailRow(Icons.phone, 'الهاتف', c.phone!),
          if (c.loginCode != null)
          ...
          _DetailRow(
              ...
              c.isBlacklisted
                  ...
                  : c.isVip
                      ? 'VIP'
                      : 'عادي'),
          _DetailRow(Icons.check,
              'مفعّل', c.isApproved ? 'نعم' : 'لا'),
          if (c.balance != 0)
              ...
              onPressed: saving
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        final code =
                            AppFormatters.generateAccessCode();
                        final customer = Customer(
                          ...
                          phone: phoneCtrl.text.trim().isEmpty
                              ...
                              : phoneCtrl.text.trim(),
                          storeType: storeType,
                          priceType: storeType == AppConstants.storeElectrical ? priceTier : AppConstants.priceRetail,
  ...
  Color _customerStatusColor(String status) {
    switch (status) {
      case 'vip': return const Color(AppColors.vipInt);
      case 'blacklist': return const Color(AppColors.blacklistInt);
      ...
      child: AnimatedContainer(
        ...
        child: Column(children: [
          ...
          Text(subtitle,
              style: TextStyle(
                  fontSize: 10,
                  color: selected ? color.withValues(alpha: 0.7) : Colors.grey)),
        ]),
      ),
    );
  }
  ...
  List<Contact> get _filtered {
    ...
    return widget.contacts
        .where((c) =>
            ...
            c.phones
                .any((p) => p.number.contains(_query)))
        .toList();
  }
