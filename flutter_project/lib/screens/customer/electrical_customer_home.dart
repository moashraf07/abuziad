        ...
        title: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (customer?.customerType == 'technician') ...[
            ...
            Container(
              ...
              child: const Text('فني', style: TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
            ),
            ...
            Container(
              ...
              child: const Text('مهندس', style: TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
            ),
          ],
        ]),
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: [const Color(AppColors.primaryInt), const Color(AppColors.primaryInt).withValues(alpha: 0.75)],
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Text('الأدوات الكهربائية', style: TextStyle(color: Colors.white70, fontSize: 13)),
        ...
        const Text('الخدمات السريعة', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        const SizedBox(height: 10),
        GridView.count(
          crossAxisCount: 2, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(),
          crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 1.4,
          children: [
            _QuickAction('تصفح المنتجات', Icons.shopping_bag, const Color(AppColors.primaryInt), onBrowseProducts),
            _QuickAction('طلب منتج', Icons.add_shopping_cart, Colors.orange,
                () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ProductRequestScreen(storeType: AppConstants.storeElectrical)))),
            _QuickAction('تسعير كشف', Icons.price_check, Colors.teal,
                () => Navigator.push(context, MaterialPageRoute(builder: (_) => _PricingSheetScreen(customer: customer)))),
          ],
        ),
      ]),
    );
...
class _ElectricalProductsTabState extends State<_ElectricalProductsTab>
  ...
  List<InstallmentProduct> _filtered(String? category) {
    ...
    if (customerTier != null && customerTier.isNotEmpty && customerTier != 'retail') {
      // Customer only sees products with no tier set OR products matching their tier
      list = list.where((p) => p.priceTier == null || p.priceTier == customerTier).toList();
    } else {
      // retail customers only see retail (null or retail) products
      list = list.where((p) => p.priceTier == null || p.priceTier == 'retail').toList();
    }
    if (category != null) {
    ...
    if (_products.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.electrical_services, size: 72, color: Colors.grey.shade300),
...
class _ElectricalProductCardState extends State<_ElectricalProductCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final hasImage =
        imgs.isNotEmpty && File(imgs.first).existsSync();
      ...
      child: InkWell(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Expanded(
            flex: 5,
            child: hasImage
                ? buildProductImage(imgs.first, fit: BoxFit.cover, width: double.infinity, height: double.infinity)
                : Container(
                    color: const Color(AppColors.primaryInt).withValues(alpha: 0.1),
                    child: const Icon(Icons.electrical_services, size: 48,
...
class _FullScreenImageViewerE extends StatefulWidget {
  ...
  const _FullScreenImageViewerE({required this.images, required this.initialIndex});
  @override
  State<_FullScreenImageViewerE> createState() => _FullScreenImageViewerEState();
}
                ...
                PageView.builder(
                  ...
                  itemBuilder: (_, i) => GestureDetector(
                    onTap: () => Navigator.push(context, MaterialPageRoute(
                      builder: (_) => _FullScreenImageViewerE(images: imgs, initialIndex: _imgIndex),
                    )),
...
class _MyElectricalRequestsTab extends StatefulWidget {
  ...
  @override
  State<_MyElectricalRequestsTab> createState() => _MyElectricalRequestsTabState();
}
...
class _MyElectricalRequestsTabState extends State<_MyElectricalRequestsTab> {
  ...
  Color _statusColor(String s) {
    switch (s) {
      case AppConstants.requestStatusApproved: return Colors.green;
      case AppConstants.requestStatusRejected: return Colors.red;
  ...
  String _statusLabel(String s) {
    switch (s) {
      case AppConstants.requestStatusApproved: return 'مقبول';
      case AppConstants.requestStatusRejected: return 'مرفوض';
  ...
  IconData _statusIcon(String s) {
    switch (s) {
      case AppConstants.requestStatusApproved: return Icons.check_circle;
      case AppConstants.requestStatusRejected: return Icons.cancel;
    ...
    if (_requests.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          const Text('لا توجد طلبات حتى الآن', style: TextStyle(color: Colors.grey, fontSize: 15)),
          const SizedBox(height: 8),
          const Text('اطلب منتجاً أو تسعير كشف من الصفحة الرئيسية', style: TextStyle(color: Colors.grey, fontSize: 12)),
        ]),
      );
    }
                ...
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    if (r.notes != null && r.notes!.isNotEmpty) ...[
                      const SizedBox(height: 4),
                      Text(r.notes!, style: const TextStyle(color: Colors.grey, fontSize: 12)),
                    ],
                  ]),
                ),
                Container(
    ...
    if (_bundles.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          const Text('لا توجد ليستات متاحة حالياً', style: TextStyle(color: Colors.grey, fontSize: 15)),
          const SizedBox(height: 8),
          const Text('سيتم إضافة الليستات قريباً', style: TextStyle(color: Colors.grey, fontSize: 12)),
        ]),
      );
    }
    return RefreshIndicator(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: Text(bundle.name, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: color))),
        ...
        if (bundle.description != null && bundle.description!.isNotEmpty)
          Padding(
            ...
            child: Text(bundle.description!, style: const TextStyle(color: Colors.grey, fontSize: 13)),
          ),
        if (bundle.items.isNotEmpty) ...[
          const Divider(height: 20, indent: 16, endIndent: 16),
          Padding(
            padding: const EdgeInsets.fromLTRB(16, 0, 16, 4),
            child: const Text('محتويات الليسته', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
          ),
          ...
          if (bundle.items.any((i) => i.originalPrice > 0)) ...[
            ...
            Padding(
              ...
              child: Builder(builder: (context) {
                final discount = bundle.discountRate ?? 0;
                final originalTotal = bundle.items.fold<double>(0, (s, i) => s + i.originalPrice);
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      for (final p in sel) {
        ...
        if (firstInsertedId == null && inserted is int && inserted > 0) firstInsertedId = inserted;
      }
      ...
      if (mounted) {
        if (invoiceId > 0) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('تم إرسال طلب التسعير كفاتورة رقم $invoiceNo'), backgroundColor: Colors.green),
          );
        } else {
          await PushNotificationService.sendToRole(
            role: 'admin',
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تم إرسال طلب التسعير بنجاح ✓'), backgroundColor: Colors.green),
          );
        }
        Navigator.pop(context);
      }
                      ...
                      child: Padding(
                        ...
                        child: Row(children: [
                          ClipRRect(
                            borderRadius: BorderRadius.circular(10),
                            child: imgPath != null
                                ? buildProductImage(imgPath, width: 60, height: 60, fit: BoxFit.cover)
                                : Container(
                                    width: 60,
                                    height: 60,
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            if (qty > 0)
                              Text('الإجمالي: ${AppFormatters.formatCurrency(price * qty)}',
                          ...
                          Row(mainAxisSize: MainAxisSize.min, children: [
                            IconButton(
                              ...
                              onPressed: qty > 0
                                  ? () => setState(() {
                                        _qty[p.id!] = qty - 1;
                                        if (_qty[p.id!]! <= 0) _qty.remove(p.id!);
                                      })
                                  : null,
                            ),
      ...
      floatingActionButton: _selected.isEmpty
          ...
          : FloatingActionButton.extended(
              ...
              icon: _submitting
                  ...
                  : const Icon(Icons.send),
              label: Text('إرسال الطلب  (${AppFormatters.formatCurrency(_total)})'),
            ),
    );
  }
}
