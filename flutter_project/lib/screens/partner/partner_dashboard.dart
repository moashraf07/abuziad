...
class _PartnerDashboardState extends State<PartnerDashboard>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        _cachedUserId = context.read<AuthProvider>().currentUser?.id;
        _load();
      }
    });
          ...
          SliverAppBar(
            ...
            bottom: PreferredSize(
              ...
              child: TabBar(
                ...
                tabs: const [
                  Tab(icon: Icon(Icons.group_outlined, size: 18), text: 'مجموعاتي'),
                  Tab(icon: Icon(Icons.bar_chart_rounded, size: 18), text: 'أرباحي'),
                  Tab(icon: Icon(Icons.inventory_2_outlined, size: 18), text: 'المنتجات'),
                  Tab(icon: Icon(Icons.credit_card_outlined, size: 18), text: 'أقساطي'),
                  Tab(icon: Icon(Icons.chat_bubble_outline, size: 18), text: 'المحادثات'),
                  Tab(icon: Icon(Icons.notifications_outlined, size: 18), text: 'الإشعارات'),
                ],
              ),
            ),
          ),
        ...
        body: _loading
            ...
            : TabBarView(
                ...
                children: [
                  ...
                  const _PartnerNotificationsTab(),
                ],
              ),
      ),
    );
...
// ─── Partner Notifications Tab ────────────────────────────────────────────────

class _PartnerNotificationsTab extends StatefulWidget {
  const _PartnerNotificationsTab();
  @override
  State<_PartnerNotificationsTab> createState() => _PartnerNotificationsTabState();
}

class _PartnerNotificationsTabState extends State<_PartnerNotificationsTab> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await _dao.getForUser(userId, role);
  ...
  Future<void> _markAllRead() async {
    ...
    final userId = auth.currentUser?.id ?? 0;
    final role = auth.currentUser?.role;
    await _dao.markAllRead(userId, role);
    _load();
  }

  IconData _iconForType(String type) {
    switch (type) {
      case 'installment': return Icons.credit_card;
              ...
              return InkWell(
                onTap: () async {
                  if (!n.isRead && n.id != null) {
                    await _dao.markRead(n.id!);
                    _load();
                  }
                },
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                  color: n.isRead ? null : const Color(AppColors.primaryInt).withValues(alpha: 0.05),
                  child: Row(children: [
                    CircleAvatar(
                      backgroundColor: n.isRead ? Colors.grey[200] : const Color(AppColors.primaryInt).withValues(alpha: 0.15),
                      child: Icon(_iconForType(n.type),
                          color: n.isRead ? Colors.grey : const Color(AppColors.primaryInt), size: 20),
                    ),
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        n.createdAt.length >= 16 ? n.createdAt.substring(0, 16).replaceAll('T', ' ') : n.createdAt,
                        style: const TextStyle(fontSize: 11, color: Colors.grey),
                      ),
                    ])),
                    if (!n.isRead)
                      Container(width: 8, height: 8,
                          decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(AppColors.accentInt))),
                  ]),
                ),
              );
            },
